...
interface Module {
  ...
  chapters?: Chapter[]; // Optional now (loaded JIT)
}
...
function RoadmapContent() {
        ...
        try {
          ...
          const res = await fetch('/api/generate-roadmap', {
            ...
            body: JSON.stringify({
              ...
              mode: "skeleton" // NEW: Request Skeleton
            })
          });
  ...
  // 3. JIT MODULE GENERATION (Unlock)
  const handleModuleClick = async (mod: Module, index: number) => {
    // A. Already Unlocked? Enter Workspace.
    if (mod.chapters && mod.chapters.length > 0) {
      setActiveModule(mod);
      return;
    }

    // B. Not Unlocked? Generate Details.
  ...
  // VIEW 2: THE ROADMAP ARCHITECTURE
  return (
