...
export default function Workspace({ module, onBack, userContext, anchorChannel, seenVideoIds, onVideoSeen }: WorkspaceProps) {
    ...
    const experience = userContext?.experience || "Deep Dive";
    ...
    useEffect(() => {
        const fetchVideo = async () => {
            ...
            try {
                ...
                if (anchorChannel) {
                    params.append('preferredChannel', anchorChannel);
                }
                ...
                if (data.densityScore !== undefined) {
                    setVideoMeta({
                        ...
                        densityFlags: data.densityFlags,
                    });
                }
                console.log(`🎬 Video selected via: ${data.source}`);
            } catch (_err) {
    ...
    return (
        <div className="w-full h-screen bg-black text-white flex flex-col pt-6 px-6 overflow-hidden relative">
            ...
            <div className="flex-1 flex flex-col lg:flex-row gap-6 overflow-hidden pb-6">
                ...
                {/* LEFT: SMART COMPANION (Tabbed) */}
                <div className="hidden lg:flex flex-[3.5] bg-gray-900/40 rounded-2xl border border-gray-800 flex-col overflow-hidden relative">
                    {/* Tab Headers */}
                    <div className="flex border-b border-gray-800">
                        <button
                            onClick={() => setCompanionTab("context")}
                            className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${companionTab === "context"
                                ? "text-teal-400 border-b-2 border-teal-500 bg-gray-800/50"
                        ...
                        <button
                            ...
                            className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${companionTab === "chat"
                                ? "text-teal-400 border-b-2 border-teal-500 bg-gray-800/50"
                                : "text-gray-500 hover:text-gray-300"
                                }`}
                        >
                    ...
                    <div className="flex-1 overflow-hidden">
                        {companionTab === "context" ? (
                            ...
                            <AIChat
                                ...
                                courseTitle={userContext?.topic || "Learning Path"}
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto max-w-[40%] custom-scrollbar">
                            {module.chapters.map((chap, i) => (
                                <div
                                    ...
                                    className={`h-2 w-8 rounded-full cursor-pointer transition-all ${i === activeChapIdx ? 'bg-teal-500' :
